...
export default function UploadGrades({ mycourses }) {
    ...
    const fetchStudents = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/students/course/${selectedCourseId}`
        );
  ...
  useEffect(() => {
    const fetchGrades = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/grades/getgrades/${selectedCourseId}`
        );
        const data = await res.json();
  ...
  const handlesubmitgrades = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/grades/transcripts/submit`,
        {
          method: "POST",
